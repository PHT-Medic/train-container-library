...
class Train:
    ...
    def load_queries(self):
        ...
        try:
            ...
            return {"1": "station_1", "2": "station_2", "3": "station_3"}
    ...
    def discovery(self, patients, results):
        ...
        results["discovery"][
            "discovery_exec_" + str(len(results["discovery"]) + 1)
        ] = discovery
        print(results)
        return results
    ...
    def plot_discovery_results(self, results):
        ...
        p2 = plt.bar(
            index, class_1, bar_width, bottom=class_0, alpha=opacity, label="NV"
        )
        p3 = plt.bar(
            index, class_2, bar_width, bottom=class_1, alpha=opacity, label="BCC"
        )
        p4 = plt.bar(
            index, class_3, bar_width, bottom=class_2, alpha=opacity, label="AK"
        )
        p5 = plt.bar(
            index, class_4, bar_width, bottom=class_3, alpha=opacity, label="BKL"
        )
        p6 = plt.bar(
            index, class_5, bar_width, bottom=class_4, alpha=opacity, label="DF"
        )
        p7 = plt.bar(
            index, class_6, bar_width, bottom=class_5, alpha=opacity, label="VASC"
        )
        p8 = plt.bar(
            index, class_7, bar_width, bottom=class_6, alpha=opacity, label="SCC"
        )
        p9 = plt.bar(
            index, class_8, bar_width, bottom=class_7, alpha=opacity, label="UNK"
        )
            ...
            for k, v in mat.items():
                if k in COLOR_DICT.keys():
                    if k not in station_dict.keys():
                        ...
                        station_dict[f"{k}_min"] = np.minimum(
                            station_dict[f"{k}_min"], v.copy()
                        )
                        station_dict[f"{k}_max"] = np.maximum(
                            station_dict[f"{k}_max"], v.copy()
                        )
    ...
    def make_plot_dict(self, station_path, plot_params, std=False, num_stations=3):
        ...
        for station in station_path:
            ...
            if not plot_dict["plot_epochs"].size == 0:
                plot_dict["plot_epochs"] = np.append(
                    plot_dict["plot_epochs"],
                    plot_dict["plot_epochs"][-1] + m["step_num"][0],
                )
            else:
                plot_dict["plot_epochs"] = np.append(
                    plot_dict["plot_epochs"], m["step_num"][0]
                )
            for k in plot_dict["params"].keys():
                if k in ["acc", "f1", "train_loss", "loss"]:
                    plot_dict["params"][k] = np.append(plot_dict["params"][k], m[k][0])
                    if std:
                        plot_dict["params"][f"{k}_min"] = np.append(
                            plot_dict["params"][f"{k}_min"], m[f"{k}_min"][0]
                        )
                        plot_dict["params"][f"{k}_max"] = np.append(
                            plot_dict["params"][f"{k}_max"], m[f"{k}_max"][0]
                        )
                elif k in ["sens", "auc"]:
                    plot_dict["params"][k] = np.append(
                        plot_dict["params"][k], np.mean(m[k], axis=1)
                    )
        return plot_dict
    ...
    def central_plot(
        ...
        if smoothing_factor:
            smoothing_factor = (
                np.ones(len(plot_params)) * smoothing_factor
                if isinstance(smoothing_factor, float)
        ...
        for i, ax in enumerate(fig.axes):
            ...
            extend_plot = (
                num_stations - plot_dict["num_stations"]
                if num_stations >= plot_dict["num_stations"]
                else plot_dict["num_stations"]
            )
            ...
            plotting = {
                item: value
                for (item, value) in plot_dict["params"].items()
                if item in plot_params
            }
            ...
            for (k, v), s in zip(plotting.items(), smoothing_factor):
                ...
                ax.plot(plot_dict["plot_epochs"], v, color=COLOR_DICT[k], label=k)
                if smoothing:
                    ax.plot(
                        plot_dict["plot_epochs"],
                        np.array(pd.Series(v).ewm(alpha=s).mean().to_numpy()),
                        color=COLOR_DICT[k],
                        label=f"{k}_smoothed",
                        alpha=0.7,
                    )
                ...
                if std:
                    ax.fill_between(
                        plot_dict["plot_epochs"],
                        plot_dict["params"][f"{k}_max"],
                        plot_dict["params"][f"{k}_min"],
                        alpha=0.2,
                        color=COLOR_DICT[k],
                    )
    ...
    def create_label_file(self, patients_df, path_labels, station):
        ...
        new_index_df.to_csv(
            path_labels + "s_" + str(station) + "_labels.csv", index=False
        )
