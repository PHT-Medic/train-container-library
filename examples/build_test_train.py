...
def build_test_train(
    master_img: str = "harbor-pht.tada5hi.net/master/python/ubuntu",
    ...
    if not (train_files or train_dir):
        raise ValueError(
            "Either files or a directory containing train files need to be set."
        )
...
def build_train_image(
    ...
    img, build_logs = client.images.build(
        fileobj=docker_file, tag=train_image_name, rm=True, pull=True
    )
    ...
    if security_protocol:
        _make_train_config(private_key, user_private_key)
...
def _make_train_config(
    private_key: Union[str, os.PathLike, BytesIO, RSAPrivateKey],
...
def _make_docker_file(
    master_img: str, executable: str, entrypoint_file: str = None
) -> BytesIO:
...
if __name__ == "__main__":
    build_test_train(
        train_dir="../train",
        entrypoint_file="/opt/pht_train/train/FHIRAverageAgeTrain.py",
    )
