...
def is_k_anonymized(df: pd.DataFrame, k: int = 3):
...
def anonymize(df: pd.DataFrame, k: int = 3, id_cols: List[str] = None, level: int = 2):
    anon_df = pd.DataFrame(columns=df.columns)

    # TODO specify id columns
...
def generalize_numeric_column(num_col: pd.Series, level: int = 2):
    return num_col
